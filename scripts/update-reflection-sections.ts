 ...
 * 
 * Run with: npx ts-node scripts/update-reflection-sections.ts
 */
...
interface DevotionReference {
  bibleText: string;
  reflectionSections: ReflectionSection[];
}

interface ReferenceData {
  [key: string]: DevotionReference;
}
...
async function updateDevotion(docId: string, referenceData: ReferenceData): Promise<void> {
  try {
    ...
    if (data.reflectionSections && data.reflectionSections.length > 0) {
      ...
      const updatedSections = data.reflectionSections.map((section: Question, index: number) => {
        return {
          ...section,
...
async function updateAllDevotions(): Promise<void> {
  try {
